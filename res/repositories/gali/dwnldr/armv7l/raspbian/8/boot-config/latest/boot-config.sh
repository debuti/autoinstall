...
alreadyinstalled() {
  result=0;
  if [[ $(ls -1 /boot/config.txt* | wc -l) -gt 1 ]]; then
    result=-1;
  fi   
  return $result;  
}
...
configure() {
  ...
  sed -i -e "/^#hdmi_group=1/s/#//" \
         -e "/hdmi_group=1/s/$/  # CEA is for TV, DMT for monitors/" \
         -e "/^#hdmi_mode=1/s/#//" \
         -e "/hdmi_mode=1/s/1/4  # 720p@60Hz/" \
         -e "/^#hdmi_force_hotplug=1/s/#//" \
         /boot/config.txt && \
  result=0
  return $result;
}
