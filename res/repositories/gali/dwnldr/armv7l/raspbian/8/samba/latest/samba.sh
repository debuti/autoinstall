...
configure() {
  ...
  cp /etc/samba/smb.conf /etc/samba/smb.conf.$(date +%Y%m%d-%H%M%S)
  echo "
# This one is for public rw access
#[root]
#   path = /
#   force user = debuti
#   force group = debuti
...
# This one is for public read access and restricted write access
[root]
